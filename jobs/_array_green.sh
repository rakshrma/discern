#!/bin/bash
#SBATCH -J metrics_green
#SBATCH --cpus-per-task=4
#SBATCH --nodes=1
#SBATCH --gpus-per-node=1
#SBATCH -t 40:00:00
#SBATCH -o logs/array_green_%A_%a.out
#SBATCH -e logs/array_green_%A_%a.err
#SBATCH --partition=dgx-b200
#
# GREEN array task — one input file per array index.
...
cd "$REPO"
echo "[GREEN] task=$SLURM_ARRAY_TASK_ID  input=$INPUT  output=$OUTPUT"
...
echo "[GREEN] subprocess timeout=${TIMEOUT}s"
...
echo "[GREEN] done task=$SLURM_ARRAY_TASK_ID"
